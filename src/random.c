...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/06 12:02:11 by lbradama          #+#    #+#             */
/*   Updated: 2019/06/23 16:51:37 by gkessler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			seeds_init(t_env *env)
{
	...
	srand((uint)clock());
	i = 0;
	while (i < env->scene->seeds->size)
	{
		env->scene->seeds->seeds[i] = (uint)rand();
		i++;
	}
}
...
float			get_random(t_uint2 *seeds)
{
	...
	ires = ((seeds->x) << 16) + (seeds->y);
	res.ui = (ires & 0x007fffff) | 0x40000000;
	return (res.f - 2.0f) / 2.0f;
}

t_vec3			ray_random(t_vec3 v, double alpha, t_uint2 *seeds)
{
	t_vec3 ray;
	double cos_theta;
	double sin_theta;
	...
	tangent_space_dir = vec3_init(cos(phi) * sin_theta,
	sin(phi) * sin_theta, cos_theta);
	ray = podrandom(v, tangent_space_dir);
	return (ray);
}
